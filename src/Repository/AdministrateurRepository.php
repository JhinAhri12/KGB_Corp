...
class AdministrateurRepository extends ServiceEntityRepository implements PasswordUpgraderInterface
{
    ...
    /**
     * Used to upgrade (rehash) the Administrateur's password automatically over time.
     */
    public function upgradePassword(UserInterface $Administrateur, string $newEncodedPassword): void
    {
        if (!$Administrateur instanceof Administrateur) {
            throw new UnsupportedAdministrateurException(sprintf('Instances of "%s" are not supported.', \get_class($Administrateur)));
        }

        $Administrateur->setPassword($newEncodedPassword);
        $this->_em->persist($Administrateur);
        $this->_em->flush();
    }
